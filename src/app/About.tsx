  ...
  return (
    <div className="min-h-screen bg-white flex flex-col">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-16 flex-1 w-full">
        {/* Page Title */}
        <div className="mb-6">
          <h2 className="mb-2 text-2xl md:text-3xl font-headline">{pageTitle}</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {aboutUs.map((item) => (
              ...
              >
                ...
                <h3 className="text-lg font-semibold mb-2 uppercase tracking-wide font-headline">{item.name}</h3>
                <p className="text-sm text-gray-600 font-body">{item.subtext}</p>
              </a>
            ))}
          </div>
        </div>
  ...
  <div className=" px-8 py-6">
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        <label
          htmlFor="email"
          className="block text-xs uppercase tracking-wider text-gray-500 mb-2 font-body"
        >
          Email
        </label>
